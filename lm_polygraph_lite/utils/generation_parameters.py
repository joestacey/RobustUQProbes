...
class GenerationParameters:
    ...
    Parameters:
        ...
        top_k (int): Top-k token predictions to consider in sampling generation. Has no effect when `do_sample` is
            not set. Default: 50.
        top_p (float): Only consider the highest unique tokens, which probabilities sum up to `top_p`. Has no effect
        ...
        presence_penalty (float): Number between -2.0 and 2.0. Positive values penalize new tokens based on whether
            they appear in the text so far, increasing the model's likelihood to talk about new topics. Default: 0.0.
        ...
        allow_newlines (bool): If set, the model is allowed to generate tokens with newlines. Default: True.
    """
