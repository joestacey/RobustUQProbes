...
from lm_polygraph_lite.utils.common import polygraph_module_init


class StatCalculator(ABC):
    """
    Abstract class for a particular statistics calculation. Used to re-use the same statistics across different
    uncertainty estimators at `lm_polygraph_lite.estimators`. See the list of available calculators at
    lm_polygraph_lite/stat_calculators/__init__.py.
    ...
    UEManager at lm_polygraph_lite.utils.manager will order all the needed calculators and estimators to be called in
    the correct order. Any cycle dependencies among calculators will be spotted by UEManager and end with an exception.

    Each new StatCalculator needs to be registered at lm_polygraph_lite/stat_calculators/__init__.py to be seen by UEManager.
    """
    ...
    def __init__(self, stats: List[str], stat_dependencies: List[str]):
        """
        Parameters:
            stats: List[str]: Names of statistics which can be calculated by using this StatCalculator.
